...
class LayeredStore implements KeyValueStore
{
    ...
    public function get(string $key, mixed &$token = null): mixed
    {
        ...
        $keyValueStoreTokens = [];
        $token = null;
        while (null === $token && false === $value && $keyValueStoreIndex < count($this->keyValueStores)) {
            $value = $this->keyValueStores[$keyValueStoreIndex]->get($key, $token);
            $keyValueStoreIndex++;
        }
    ...
    public function set(string $key, mixed $value, int $expire = 0): bool
    {
        ...
        if (!$result) {
            ...
            while ($keyValueStoreIndex < count($this->keyValueStores)) {
                $this->keyValueStores[$keyValueStoreIndex++]->delete($key);
            }
        }
